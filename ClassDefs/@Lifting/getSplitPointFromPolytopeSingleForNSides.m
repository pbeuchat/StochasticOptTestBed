function [deltaMin, deltaMax, deltaSplitPoint] = getSplitPointFromPolytopeSingleForNSides(S, h, NSides)
...
%  x        - is the variable in the original space
%  n        - is the size of "x"
%  S,h      - describe the \mathcal{X} polytopic set in the original space
%               "S x <= h"

%% EXTRACT THE ORIGINAL SIZE FROM THE INPUT "SSingle" MATRIX
n_single = size(SSingle,2);


%% Find the MINIMUM and MAXIMUM "x" for each component of "x"
% ie. solve:        min_x{ e_i^T x : s.t. S x <= h }  =  x_{i,min}
%     and also:     max_x{ e_i^T x : s.t. S x <= h }  =  x_{i,max}
[ x_min , x_max ] = getElementwiseMaxAndMinForSet(S, h);


%% NOTE: that the "Sx<=h" polytope may 
