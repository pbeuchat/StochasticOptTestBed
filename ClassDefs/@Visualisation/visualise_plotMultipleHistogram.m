function [ ] = visualise_plotMultipleHistogram( hAxes , data_y , varargin )
...
%     end
    ...
    % Line defaults
    default_lineColourIndex = ones( numHistToPlot , 1 );
    default_lineWidth = Visualisation.lineWidthDefault * ones( numHistToPlot , 1 );
    default_markerIndex = ones( numHistToPlot , 1 );
            ...
            switch lower(inputSettings{iTemp,1})
                ...
                case 'linecolourindex'
                    ...
                    if ( isscalar(lineColourIndex) && ismatrix(lineColourIndex) )
                        lineColourIndex = repmat(lineColourIndex,numHistToPlot,1);
                    end
                    % Check it is the right size
                    if ~( length(lineColourIndex) == numHistToPlot && isvector(lineColourIndex) )
                ...
                case 'linewidth'
                    ...
                    if ( isscalar(lineWidth) && ismatrix(lineWidth) )
                        lineWidth = repmat(lineWidth,numHistToPlot,1);
                    end
                    % Check it is the right size
                    if ~( length(lineWidth) == numHistToPlot && min(size(lineWidth)) == 1 && ismatrix(lineWidth) )
                ...
                case 'markerindex'
                    ...
                    if ( isscalar(markerIndex) && ismatrix(markerIndex) )
                        markerIndex = repmat(markerIndex,numHistToPlot,1);
                    end
                    % Check it is the right size
                    if ~( length(markerIndex) == numHistToPlot && min(size(markerIndex)) == 1 && ismatrix(markerIndex) )
                ...
                case 'legendstrings'
                    ...
                    if ~isempty(legendStrings)
                        ...
                        % Check it is the right size
                        if ~( length(legendStrings) == numHistToPlot && isvector(legendStrings) )
    ...
    hHist = cell(numHistToPlot,1);
    
    % Plot the scatter data
    hold on;
    for iLine = 1:numHistToPlot
        % Get the colour for this line
        thisColour = Visualisation.getDefaultColourForIndex( lineColourIndex(iLine) );
        
        % Specify a few things
        thisEdgeColour = [0,0,0];
        thisFaceAlpha = 0.5;
        thisOrientation = 'horizontal';
        
        
        % Plot the line
        if iscell(data_y)
            %hLine(iLine) = plot( hAxes, data_x{iLine}, data_y{iLine} , 'color' , thisColour , 'LineWidth' , thisLineWidth , 'marker' , thisMarker);%
            hHist{iLine,1} = histogram( hAxes, data_y{iLine,1} , 'FaceColor' , thisColour , 'EdgeColor' , thisEdgeColour , 'FaceAlpha' , thisFaceAlpha, 'Orientation', thisOrientation );
        else
            hHist{iLine,1} = histogram( hAxes, data_y(iLine,:), 'FaceColor' , thisColour , 'EdgeColor' , thisEdgeColour , 'FaceAlpha' , thisFaceAlpha, 'Orientation', thisOrientation );
        end
    end
    hold off;
    

    % Get the minimum bin width
    minBinWidth = inf;
    for iLine = 1:numHistToPlot
        minBinWidth = min( hHist{iLine,1}.BinWidth , minBinWidth );
    end
    % Now set them all to this width
    for iLine = 1:numHistToPlot
        hHist{iLine,1}.BinWidth = minBinWidth;
    end
    ...
    %set(hAxes,'XTick',1:numHistToPlot);
    ...
    if ~isempty(legendStrings) && strcmp(legendOnOff,'on')
        ...
        for iLine = 1:numHistToPlot
            hHist{iLine,1}.DisplayName = legendStrings{iLine,1};
        end
