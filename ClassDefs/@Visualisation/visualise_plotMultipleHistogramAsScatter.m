function [ ] = visualise_plotMultipleHistogramAsScatter( hAxes , data_y , varargin )
...
%     end
    ...
    if ~iscell(data_y)
        numHistScatterToPlot = size(data_y,1);
    else
        numHistScatterToPlot = size(data_y,1);
    end
    ...
    % Line defaults
    default_lineColourIndex = ones( numHistScatterToPlot , 1 );
    default_lineWidth = Visualisation.lineWidthDefault * ones( numHistScatterToPlot , 1 );
    default_markerIndex = ones( numHistScatterToPlot , 1 );
            ...
            switch lower(inputSettings{iTemp,1})
                ...
                case 'linecolourindex'
                    ...
                    if ( isscalar(lineColourIndex) && ismatrix(lineColourIndex) )
                        lineColourIndex = repmat(lineColourIndex,numHistScatterToPlot,1);
                    end
                    % Check it is the right size
                    if ~( length(lineColourIndex) == numHistScatterToPlot && isvector(lineColourIndex) )
                ...
                case 'linewidth'
                    ...
                    if ( isscalar(lineWidth) && ismatrix(lineWidth) )
                        lineWidth = repmat(lineWidth,numHistScatterToPlot,1);
                    end
                    % Check it is the right size
                    if ~( length(lineWidth) == numHistScatterToPlot && min(size(lineWidth)) == 1 && ismatrix(lineWidth) )
                ...
                case 'markerindex'
                    ...
                    if ( isscalar(markerIndex) && ismatrix(markerIndex) )
                        markerIndex = repmat(markerIndex,numHistScatterToPlot,1);
                    end
                    % Check it is the right size
                    if ~( length(markerIndex) == numHistScatterToPlot && min(size(markerIndex)) == 1 && ismatrix(markerIndex) )
                ...
                case 'legendstrings'
                    ...
                    if ~isempty(legendStrings)
                        ...
                        % Check it is the right size
                        if ~( length(legendStrings) == numHistScatterToPlot && isvector(legendStrings) )
    ...
    %% ----------------------------------------------------------------- %%
    %% CREATE A SERIES OF X-VECTORS TO PLOT THE SCATTER DATA AGAINST
    
    
    
    
    if iscell(data_y)
        data_x = cell( numHistScatterToPlot , 1 );
        for iLine = 1:numHistScatterToPlot
            data_x{iLine,1} = iLine * ones( 1 , size( data_y{iLine,1} , 2 ) );
        end
    else
        histScatterLength = size( data_y , 2 );
        data_x = repmat( (1 : numHistScatterToPlot)' , 1 , histScatterLength );
    end
    
    
    x_limit_min = 0;
    x_limit_max = numHistScatterToPlot + 1;
    
    
    
    %% ----------------------------------------------------------------- %%
    %% COMPUTE THE STATISTICS TO PLOT THE MEAN AND VARIANCE
    
    if iscell(data_y)
        data_mean_y = zeros( numHistScatterToPlot , 3 );
        for iLine = 1:numHistScatterToPlot
            data_mean_y(iLine,:) = mean(data_y{iLine,1},2);
        end
    else
        data_mean_y = repmat( mean(data_y,2) , 1 , 3 );
    end
    
    data_mean_x = [ (1 : numHistScatterToPlot)' - 0.4  , ...
                    (1 : numHistScatterToPlot)' - 0.2  , ...
                    (1 : numHistScatterToPlot)' + 0.4   ...
                  ];
    
    flag_std = 0;       % OPTIONS: '0' normalises by n, and '1' by (n-1)
    if iscell(data_y)
        data_mean_std = zeros( numHistScatterToPlot , 3 );
        for iLine = 1:numHistScatterToPlot
            data_mean_std(iLine,2) = std(data_y{iLine,1},flag_std,2);
        end
    else
        data_mean_std = [ 0 , std(data_y,flag_std,2) , 0 ];
    end
    ...
    hScatter = zeros(numHistScatterToPlot,1);
    hLine = zeros(numHistScatterToPlot,1);
    ...
    for iLine = 1:numHistScatterToPlot
        ...
        if iscell(data_y)
            ...
            hScatter(iLine) = scatter( hAxes, data_x{iLine,:}, data_y{iLine,1} , 'MarkerEdgeColor' , thisColour , 'LineWidth' , thisLineWidth , 'marker' , thisMarker);
        else
            hScatter(iLine) = scatter( hAxes, data_x(iLine,:), data_y(iLine,:) , 'MarkerEdgeColor' , thisColour , 'LineWidth' , thisLineWidth , 'marker' , thisMarker);
        end
    end
    hold off;
    ...
    set(hAxes,'XTick',1:numHistScatterToPlot);
