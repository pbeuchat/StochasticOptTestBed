function [ ] = visualise_plotParetoFrontSummary( hAxes , data_x , data_y , varargin )
...
%     end
            ...
            switch lower(inputSettings{iTemp,1})
                ...
                case 'legendstrings'
                    ...
                    if ~isempty(legendStrings)
                        ...
                        % Check it is the right size
                        if ~( (length(legendStrings) == numScatterToPlot || length(legendStrings) == 1) && isvector(legendStrings) )
    ...
    for iLine = 1:numScatterToPlot
        ...
        % Plot a scatter of the mean point for this set of scatter points
        hScatter_mean{iLine,1} = scatter( hAxes, data_x_mean(iLine,1), data_y_mean(iLine,1) , 'MarkerEdgeColor' , thisColour , 'LineWidth' , 2*thisLineWidth , 'SizeData' , 72*2 , 'marker' , 'x');
    end
    
    % Plot a line joining the mean points
    hLine_mean{1,1} = plot( hAxes, data_x_mean(:,1), data_y_mean(:,1) , 'LineWidth' , Visualisation.lineWidthDefault , 'Color' , thisColour);
...
%     hold off;
    ...
    if ~isempty(legendStrings) && strcmp(legendOnOff,'on')
        ...
        % Set the display name to blank and turn the legend OFF for all the
        % scatters
        for iLine = 1:numScatterToPlot
            hScatter{iLine,1}.DisplayName = '';
            hScatter{iLine,1}.Annotation.LegendInformation.IconDisplayStyle = 'off';        % OPTIONS: 'on' , 'off' , 'children'
        end
        for iLine = 1:numScatterToPlot
        ...
        % Only turn the legend on for the mean line
        hLine_mean{1,1}.DisplayName = legendStrings{1,1};
        hLine_mean{1,1}.Annotation.LegendInformation.IconDisplayStyle = 'on';        % OPTIONS: 'on' , 'off' , 'children'
